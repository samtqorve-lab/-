...
/**
 * یک ورودی رمز عبور به‌همراه دکمه‌ی «نمایش/مخفی‌کردن» (👁) می‌سازد. خروجی یک wrapper است که خود
 * input رمز داخلش قرار دارد — attrs همان چیزی است که به خود <input> پاس داده می‌شود (مثلاً
 * placeholder, autocomplete, dir).
...
export function fmtDateTime(d) {
  if (!d) return '—';
  try { return new Date(d).toLocaleString('fa-IR'); } catch { return '—'; }
}
