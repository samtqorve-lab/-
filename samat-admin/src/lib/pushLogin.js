...
export async function requestPushApproval(email, onResolve, onAwaitingCode) {
  ...
  // شبکه‌ی محافظ نهایی: قبلاً این سقف ۳۰ ثانیه بود — اما یک کاربر واقعی که تازه کد فال‌بک از
  // طریق تلگرام دریافت کرده، باید اپ پیام‌رسان را باز کند، کد را پیدا/کپی کند و به این صفحه
  // برگردد؛ این چرخه معمولاً بیشتر از ۳۰ ثانیه طول می‌کشد. نتیجه این بود که signOut از اینجا
  // (finish('error', ...)) همیشه *قبل* از اینکه کاربر فرصت واردکردن کد را داشته باشد فعال
  // می‌شد — یعنی یک کد کاملاً درست و تازه‌ارسالی، بعداً با خطای گمراه‌کننده‌ی «کد اشتباه است»
  // (در واقع ۴۰۱ ناشی از session باطل‌شده) رد می‌شد. سقف تازه (۳ دقیقه) با مهلت واقعی کد
  // (CODE_VALID_MS = ۵ دقیقه) هم‌خوان‌تر است و هنوز هم رابط‌کاربری را برای همیشه معلق نمی‌گذارد.
  setTimeout(() => { if (!settled) finish('error', 'اتصال به سرور برقرار نشد — دوباره تلاش کنید'); }, 180000);
