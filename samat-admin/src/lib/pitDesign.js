...
//
// اتصال به بقیهٔ ابزارهای معدنی این پروژه (نه فقط اعداد پیش‌فرض ثابت):
//  - عرض رمپ می‌تواند از روی عرض واقعیِ دامپتراک انتخاب‌شده (equipmentSpecs.js) با
//    suggestRampWidth پیشنهاد شود، به‌جای یک عدد حدسی.
//  - ارتفاع پله می‌تواند از روی حداکثر ارتفاع دسترسی بیل/لودر انتخاب‌شده
//    (equipmentSpecs.js, فیلد maxBenchHeightM) پیشنهاد شود.
//  - شیب سینه/برم می‌تواند با نتیجهٔ رده‌بندی RMR واقعیِ سایت (geologyCalc.js:
...
// الگویی است. بررسی پایداری واقعی از طریق پیوند با geologyCalc.js/miningEconomics.js
// در pitDesignPage.js انجام می‌شود، اما آن هم یک غربالگری اولیه است، نه جایگزین
// بررسی مهندس ژئوتکنیک/معدن با داده‌ی میدانی کامل و متن دقیق مقررهٔ حاکم.
//
...
/**
 * عرض جادهٔ دسترسی (رمپ) پیشنهادی از روی عرض واقعی یک دامپتراک (equipmentSpecs.js: widthM).
 * قاعدهٔ سرانگشتی متداول در طراحی جادهٔ معدنی: جادهٔ یک‌طرفه ≈ ۲.۵× عرض کامیون + شانه‌ها،
 * جادهٔ دوطرفه ≈ ۳.۵× عرض کامیون (این نسخهٔ ساده‌شده است؛ طراحی کامل جادهٔ معدنی سرعت طراحی،
 * شیب عرضی/superelevation، خندق و دید در پیچ را هم لحاظ می‌کند — مثلاً طبق راهنمای Caterpillar
 * برای ناوگان کامیون‌های خارج‌جاده‌ای — که اینجا پیاده نشده است).
 * @param {number} vehicleWidthM
 * @param {1|2} [lanes] یک‌طرفه یا دوطرفه
 * @returns {number} عرض پیشنهادی جاده به متر
 */
