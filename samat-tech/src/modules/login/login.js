...
export function mountLogin(root, onSuccess) {
  ...
  function drawLogin(card) {
    ...
    const codeBox = el('div', { style: 'display:none;margin-top:14px;padding-top:14px;border-top:1px dashed var(--stone-200)' }, [
      el('div', { style: 'font-size:12px;color:var(--stone-600);margin-bottom:8px' },
  ...
  function drawRegister(card) {
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      // ربات تلگرام فقط با آیدی عددی چت می‌تواند پیام خصوصی بفرستد، نه یوزرنیم @ — بدون این
      // بررسی، ارسال کد ورود/یادآوری بی‌صدا (بدون خطای قابل‌مشاهده در همان لحظه) شکست می‌خورد.
