 ...
 * ادمین (جدول login_approvals و Edge Functionhttpsای push-login-notify/push-login-fallback مشترک
 ...
 * بماند)، این تماس برای همیشه معلق می‌ماند. با timeout مستقیم روی خودِ fetch، هر فراخوانی این
 * تابع (هرجا که باشد) محافظت دارد.
 */
...
export async function requestPushApproval(email, onResolve, onAwaitingCode) {
  ...
  // ثانیه finish('error') را صدا می‌زد — حتی وقتی کد تلگرام قبلاً با موفقیت ارسال شده بود و کاربر
  // داشت درست در مهلت واقعی ۵دقیقه‌ای واردش می‌کرد — یعنی کاربر عملاً فقط ۳۰ ثانیه (نه ۵ دقیقه)
  // فرصت داشت کد را وارد کند — و دکمه‌ی «ارسال دوباره‌ی کد» هم بعد از آن همیشه شکست می‌خورد (چون
  // settled قبلاً true شده بود). حالا در همان متغیری نگه‌داری می‌شود که armCodeTimer پاک می‌کند، پس
  // به‌محض رسیدن به مرحله‌ی «در انتظار کد»، این تایمر ۳۰ثانیه بی‌اثر می‌شود و فقط مهلت ۵
  // دقیقه‌ای واقعی حاکم است.
  ...
  // اگر تماس notify زیر دچار تأخیر/گیر شبکه شود، نباید لحظه‌ی تایید‌شدن را از دست بدهیم.
  channel = sb
  ...
  // شبکه‌ی محافظ نهایی: فقط برای مرحله‌ی «قبل از رسیدن به کد» است (یعنی اگر notify/فال‌بک
  // هردو در همان ابتدا برای همیشه گیر کنند). به‌محض اینکه armCodeTimer صدا زده شود (کد با موفقیت
  // ارسال شد)، این تایمر پاک می‌شود و دیگر اثری ندارد — پس کاربر واقعاً ۵ دقیقه‌ی کامل CODE_VALID_MS
  // را برای واردکردن کد در اختیار دارد، نه فقط تا این ۳۰ ثانیه.
